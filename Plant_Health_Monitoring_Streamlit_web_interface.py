...
# Sample data in a dictionary format for plant health status
data = {
    'Status': ['Healthy', 'Rust', 'Powdery Mildew'],
    'Reason': ['Optimal temperature and humidity', 
               'Fungal infection due to excess moisture', 
               'High humidity with poor ventilation'],
    'Prevention': ['Maintain balanced soil moisture and provide proper light',
                   'Avoid overhead watering and ensure proper air circulation',
                   'Use fungicides and improve airflow around plants']
}
...
# Streamlit app title
st.title("Leaf Disease Classification and Plant Health Status Table")
st.write("Upload an image of a leaf to classify it as Healthy, Powdery, or Rust.")
...
if uploaded_file is not None:
    ...
    # Map predicted class to the label
    ...
    if predicted_label == 'Powdery':
        predicted_label = 'Powdery Mildew'  # Adjusting to match the DataFrame status

    filtered_df = df[df['Status'] == predicted_label]
